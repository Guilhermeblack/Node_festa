...
controller.novo = async (req, res) => {
   try {
      await Ingressos.create(req.body)
...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) { // Se houver query string
      ...
      try {
         ...
         const lista = await Ingressos.find().populate(
            { path: 'portaria_ingresso', select: 'nome RG'}
       )
...
controller.obterUm = async (req, res) => {
   ...
   try {
      ...
      const obj = await Ingressos.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Ingressos.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Ingressos.findByIdAndDelete(id)
      if (obj) {
         res.status(204).end()
      }
      else {
...
async function busca(req, res) {
   ...
   try {
      const lista = await Ingressos.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
