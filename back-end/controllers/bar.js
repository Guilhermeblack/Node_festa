...
controller.obterUm = async (req, res) => {
   try {
      ...
      const obj = await Bar.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Bar.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Bar.findByIdAndDelete(id)
      if(obj) {
         res.status(204).end()
      }
      else {
...
async function busca (req, res) {
   ...
   try{
      let lista = await Bar.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
